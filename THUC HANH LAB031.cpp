...
// Ghi danh sach sinh vien vao file nhi phan
void GhiFile(SinhVien* dau, char tenFile[]) {
    FILE* f = fopen(tenFile, "wb");
    while (dau != NULL) {
        fwrite(dau, sizeof(SinhVien) - sizeof(SinhVien*), 1, f); 
        dau = dau->next;
    }
    fclose(f);
...
// Doc danh sach sinh vien tu file nhi phan (da sua dung cach)
void DocFile(SinhVien** dau, char tenFile[]) {
    ...
    SinhVien temp;
    while (fread(&temp, sizeof(SinhVien) - sizeof(SinhVien*), 1, f)) {
        SinhVien* svMoi = (SinhVien*)malloc(sizeof(SinhVien));
        if (svMoi == NULL) {
    ...
    do {
        ...
        switch (luaChon) {
            ...
            case 7:
                ...
                break;
            case 8:
                GhiFile(danhSach, "sinhvien.dat"); break;
            case 9:
                DocFile(&danhSach, "sinhvien.dat"); break;
            case 0:
